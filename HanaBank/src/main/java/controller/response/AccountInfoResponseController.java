...
@WebServlet({"/accounts-response", "/username-response", "/my-accounts-response",
        "/registrate-response", "/balance-response"})
public class AccountInfoResponseController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (command.equals("/accounts-response")) {
            ...
            String memberId = memberService.findMemberIdByPersonalIdNumber(personalIdNumber);
            System.out.println(memberId);
            List<AccountInfoDTO> accountInfos =
                    accountInfoService.findMyAccountsByMemberId(memberId);
    ...
    protected void doPut(HttpServletRequest request, HttpServletResponse response)
        ...
        if (command.equals("/registrate-response")) {
            ...
            try {
                ...
                AccountUpdateInfoDTO[] accountInfoArray =
                        gson.fromJson(data, AccountUpdateInfoDTO[].class);

                // 각 AccountUpdateInfoDTO 객체에 대해 DB 업데이트
                for (AccountUpdateInfoDTO accountInfo : accountInfoArray) {
                    accountInfoService.registrateOpenbanking(accountInfo.getAccountNumber(),
                ...
                response.getWriter().write(
                        "{\"message\":\"Failed to update accounts: " + e.getMessage() + "\"}");
            }
        }
    }
