...
<%
   ...
   %>
	...
	<script>
		...
		console.log(accountNumber + " " + bankCode + " " + nickname);
		$(document)
				.ready(
						function() {
							$
									.ajax({
										...
										success : function(response) {
											var titleElement = document
													.querySelector('.title');
											titleElement.append(nickname, " 계좌의 거래내역");
											...
											for (var i = 0; i < transactionList.length; i++) {
												...
												tr
														.append(
																"<td>"
																		+ transactionList[i].transferId
																		+ "</td>")
														.append(
																"<td>"
																		+ addBankName(transactionList[i].accountNumber1)
																		+ "</td>")
														.append(
																"<td>"
																		+ addBankName(transactionList[i].accountNumber2)
																		+ "</td>")
														.append(
																"<td>"
																		+ transactionList[i].inoutType
																		+ "</td>")
														.append(
																"<td>"
																		+ transactionList[i].tranAmt
																		+ "</td>")
														.append(
																"<td>"
																		+ transactionList[i].content
																		+ "</td>")
														.append(
																"<td>"
																		+ transactionList[i].tranDate
																		+ "</td>")
														.append(
																"<td>"
																		+ transactionList[i].tranTime
																		+ "</td>");
												tbody.append(tr);
											}
										},
		...
		function addBankName(accountNumber) {
		     ...
		     if (accountNumber.startsWith('001')) {
		       ...
		       modifiedAccountNumber = '기업 ' + accountNumber;
		     } else {
	...
	<footer class="BankFooter">
		<div class="BankFooterContent">
			...
			<p class="FooterDescription">기업은행오픈뱅킹입니다.</p>
		</div>
